"""
Script migration để thêm cột full_name vào bảng user
...
try:
    ...
    if 'full_name' in columns:
        print("✅ Cột 'full_name' đã tồn tại trong database")
    else:
        # Thêm cột mới
        cursor.execute("ALTER TABLE user ADD COLUMN full_name VARCHAR(100)")
        conn.commit()
        print("✅ Đã thêm cột 'full_name' vào bảng user thành công!")
    
    # Kiểm tra lại
    cursor.execute("PRAGMA table_info(user)")
    print("\n📋 Cấu trúc bảng user hiện tại:")
